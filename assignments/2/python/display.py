...
def gifwrite(frames: List[pg.Surface], filename: str):
    with imageio.get_writer(filename, duration=TIME_STEP) as writer:
        for frame in frames:
            array = pg.surfarray.pixels3d(frame).swapaxes(0, 1)
            writer.append_data(array)


def animate(h: Trajectory, filename: str = None):
    with PyGameDisplay() as screen:
        frames = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--position', type=float, default=0.)
